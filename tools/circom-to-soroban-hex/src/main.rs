use ark_bn254::{Fq, Fq2, G1Affine, G2Affine, Fr};
...
fn g1_bytes(x: &str, y: &str) -> Vec<u8> {
    let x_fq = Fq::from_str(x).expect("invalid G1 x");
    let y_fq = Fq::from_str(y).expect("invalid G1 y");
    let p = G1Affine::new(x_fq, y_fq);
    let mut out = Vec::new();
...
fn public_hex(path: &str) -> String {
    ...
    for s in signals {
        let fr = Fr::from_str(&s).expect("invalid field element");
        let mut bytes = [0u8; 32];
        fr.serialize_uncompressed(&mut bytes[..]).expect("failed to serialize Fr");
        out.extend(bytes);
    }
...
fn main() {
    ...
    let kind = args.next().expect("usage: circom-to-soroban-hex <vk|proof|public> <json-file>");
    let path = args.next().expect("usage: circom-to-soroban-hex <vk|proof|public> <json-file>");
